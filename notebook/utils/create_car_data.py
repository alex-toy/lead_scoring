...
from utils.mltask import MLTask


def load_car_data():
    try:
        dir_path = "/".join(os.path.dirname(os.path.realpath(__file__)).split("/")[:-1])
        data = pd.read_csv(os.path.join(dir_path, 'data/carInsurance_train.csv'))
        logging.info('Data were correctly read.')
    ...
    dataset = Bunch(data=(data.drop('CarInsurance', axis=1)), target=np.array(data['CarInsurance']))
    return dataset
